...
describe Player do
  ...
  describe '#get_symbol' do
    ...
    context 'When valid symbol is given' do
      it 'sets player.symbol to valid symbol' do
        valid_symbol = '@'
        allow(player).to receive(:gets).and_return(valid_symbol)
        player.get_symbol
        expect(player.symbol).to eq(valid_symbol)
      end
    end
  end
end
